...
This project uses 4 specialized agents for automated domain testing, with a validation script as a quality gate between stages.

## Agent: crawl-explorer
- **File:** [.github/agents/crawl-explorer.md](.github/agents/crawl-explorer.md)
- **Role:** Navigate target website using Playwright MCP, discover page sections, prove every interaction with executable evidence (Element Proof Protocol, DOM Diffing, Assertion Dry Runs), and document findings with Interaction Recipes
...
- **Role:** Re-crawl pages to verify spec accuracy using goal-backward verification, re-execute every Interaction Recipe including Assertion Commands
- **Tools:** All Playwright MCP browser tools + file system
...
- **Role:** Set up domain-specific test files (POMs, fixtures, helpers) in `src/`, maps every Interaction Recipe to POM methods including Assertion Commands
...
- **Role:** Write Playwright test cases from verified specs using established architecture, with structured Failure Diagnosis Protocol (max 3 retries per test before escalating)
- **Tools:** Playwright MCP browser tools + file system
